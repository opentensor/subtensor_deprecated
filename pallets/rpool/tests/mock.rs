...
use pallet_rpool::{NeuronMetadata};
...
frame_support::construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		Rpool: pallet_rpool::{Pallet, Call, Config, Storage, Event<T>},
	}
);

#[allow(dead_code)]
pub type RpoolCall = pallet_rpool::Call<Test>;
...
parameter_types! {
	...
	pub const RpoolSDebug:u64 = 1;
	pub const RpoolInitialRho: u64 = 10;
	pub const RpoolInitialKappa: u64 = 2;
	pub const RpoolSelfOwnership: u64 = 2;
	pub const RpoolInitialImmunityPeriod: u64 = 2;
	pub const RpoolInitialMaxAllowedUids: u64 = 100;
	pub const RpoolInitialBondsMovingAverage: u64 = 500_000;
	pub const RpoolInitialIncentivePruningDenominator: u64 = 1;
	pub const RpoolInitialStakePruningDenominator: u64 = 1;
	pub const RpoolInitialFoundationDistribution: u64 = 0;

	pub const RpoolInitialValidatorBatchSize: u64 = 10;
	pub const RpoolInitialValidatorSequenceLen: u64 = 10;
	pub const RpoolInitialValidatorEpochLen: u64 = 10;
	pub const RpoolInitialValidatorEpochsPerReset: u64 = 10;

	pub const RpoolInitialMinAllowedWeights: u64 = 0;
	pub const RpoolInitialMaxAllowedMaxMinRatio: u64 = 0;
	pub const RpoolInitialBlocksPerStep: u64 = 1;
	pub const RpoolInitialIssuance: u64 = 548833985028256;
	pub const RpoolInitialDifficulty: u64 = 10000;
	pub const RpoolMinimumDifficulty: u64 = 10000;
	pub const RpoolInitialActivityCutoff: u64 = 5000;
	pub const RpoolMaximumDifficulty: u64 = u64::MAX/4;
	pub const RpoolInitialAdjustmentInterval: u64 = 100;
	pub const RpoolInitialMaxRegistrationsPerBlock: u64 = 2;
	pub const RpoolInitialTargetRegistrationsPerInterval: u64 = 2;
}
...
impl pallet_rpool::Config for Test {
	type Event = ();
	type RpoolSDebug = RpoolSDebug;
	type RpoolInitialRho = RpoolInitialRho;
	type RpoolInitialKappa = RpoolInitialKappa;
	type RpoolSelfOwnership = RpoolSelfOwnership;
	
	type RpoolInitialValidatorBatchSize = RpoolInitialValidatorBatchSize;
	type RpoolInitialValidatorSequenceLen = RpoolInitialValidatorSequenceLen;
	type RpoolInitialValidatorEpochLen = RpoolInitialValidatorEpochLen;
	type RpoolInitialValidatorEpochsPerReset = RpoolInitialValidatorEpochsPerReset;

	type RpoolInitialImmunityPeriod = RpoolInitialImmunityPeriod;
	type RpoolInitialMaxAllowedUids = RpoolInitialMaxAllowedUids;
	type RpoolInitialMinAllowedWeights = RpoolInitialMinAllowedWeights;
	type RpoolInitialBondsMovingAverage = RpoolInitialBondsMovingAverage;
	type RpoolInitialMaxAllowedMaxMinRatio = RpoolInitialMaxAllowedMaxMinRatio;
	type RpoolInitialStakePruningDenominator = RpoolInitialStakePruningDenominator;
	type RpoolInitialIncentivePruningDenominator = RpoolInitialIncentivePruningDenominator;
	type RpoolInitialFoundationDistribution = RpoolInitialFoundationDistribution;
	type RpoolInitialIssuance = RpoolInitialIssuance;
	type RpoolInitialDifficulty = RpoolInitialDifficulty;
	type RpoolMinimumDifficulty = RpoolMinimumDifficulty;
	type RpoolMaximumDifficulty = RpoolMaximumDifficulty;
	type RpoolInitialBlocksPerStep = RpoolInitialBlocksPerStep;
	type RpoolInitialActivityCutoff = RpoolInitialActivityCutoff;
	type RpoolInitialAdjustmentInterval = RpoolInitialAdjustmentInterval;
	type RpoolInitialMaxRegistrationsPerBlock = RpoolInitialMaxRegistrationsPerBlock;
	type RpoolInitialTargetRegistrationsPerInterval = RpoolInitialTargetRegistrationsPerInterval;
}
...
pub fn register_ok_neuron( hotkey_account_id: u64, coldkey_account_id: u64) -> NeuronMetadata<u64> {
	let block_number: u64 = Rpool::get_current_block_as_u64();
	let (nonce, work): (u64, Vec<u8>) = Rpool::create_work_for_block_number( block_number, (hotkey_account_id + coldkey_account_id) * 1000000 );
	let result = Rpool::register( <<Test as frame_system::Config>::Origin>::signed(hotkey_account_id), block_number, nonce, work, hotkey_account_id, coldkey_account_id );
	assert_ok!(result);
...
pub fn register_ok_neuron_with_nonce( hotkey_account_id: u64, coldkey_account_id: u64, nonce: u64 ) -> NeuronMetadata<u64> {
	let block_number: u64 = Rpool::get_current_block_as_u64();
	let (nonce2, work): (u64, Vec<u8>) = Rpool::create_work_for_block_number( block_number, nonce );
	let result = Rpool::register( <<Test as frame_system::Config>::Origin>::signed(hotkey_account_id), block_number, nonce2, work, hotkey_account_id, coldkey_account_id );
	assert_ok!(result);
	let neuron = Rpool::get_neuron_for_hotkey(&hotkey_account_id);
	neuron
}
...
pub fn serve_axon( hotkey_account_id : u64, version: u32, ip: u128, port: u16, ip_type : u8, modality: u8 ) -> NeuronMetadata<u64> {
	let result = Rpool::serve_axon(<<Test as frame_system::Config>::Origin>::signed(hotkey_account_id), version, ip, port, ip_type, modality );
	assert_ok!(result);
	let neuron = Rpool::get_neuron_for_hotkey(&hotkey_account_id);
	neuron
}
...
pub(crate) fn run_to_block(n: u64) {
    while System::block_number() < n {
		Rpool::on_finalize(System::block_number());
        System::on_finalize(System::block_number());
        System::set_block_number(System::block_number() + 1);
        System::on_initialize(System::block_number());
		Rpool::on_initialize(System::block_number());
    }
}
