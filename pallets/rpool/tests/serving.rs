...
fn test_serve_ok_dispatch_info_ok() {
	new_test_ext().execute_with(|| {
		...
		let ip_type = 4;
        let modality = 0;
        let call = Call::Rpool(RpoolCall::serve_axon{version, ip, port, ip_type, modality});
...
fn test_serve_not_registered() {
	new_test_ext().execute_with(|| {
		...
		let result = Rpool::serve_axon(<<Test as Config>::Origin>::signed(hotkey), version, ip, port, ip_type, modality );
...
fn test_serve_success() {
	new_test_ext().execute_with(|| {
		...
		register_ok_neuron(hotkey, coldkey);
		assert_ok!(Rpool::serve_axon(<<Test as Config>::Origin>::signed(hotkey), version, ip, port, ip_type, modality ));
        let neuron = Rpool::get_neuron_for_hotkey(&hotkey);
...
fn test_serve_success_with_update() {
	new_test_ext().execute_with(|| {
		...
		let port_2 = 1337;
		let modality_2 = 0;
        assert_ok!(Rpool::serve_axon(<<Test as Config>::Origin>::signed(hotkey), version_2, ip_2, port_2, ip_type_2, modality_2 ));
        let neuron2 = Rpool::get_neuron_for_hotkey(&hotkey);
        ...
        // Check if this function works
		assert_eq!(Rpool::is_uid_active(neuron2.uid), true);

		// Check neuron count increment functionality
        assert_eq!(Rpool::get_neuron_count(), 1);

		// Check if weights are set correctly. Only self weight
		assert_eq!( Rpool::get_weights_for_neuron(&neuron2), vec![u32::MAX] );

		// Check if the neuron has a hotkey account
		assert_eq!(Rpool::has_hotkey_account(&neuron2.uid), true);

		// Check if the balance of this hotkey account == 0
		assert_eq!(Rpool::get_stake_of_neuron_hotkey_account_by_uid(neuron2.uid), 0);
